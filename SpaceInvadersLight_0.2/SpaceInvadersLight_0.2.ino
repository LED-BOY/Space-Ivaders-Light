/*SpaceLight with Clock for LEDBOY and any Attiny series 1 compatible using neopixels
...
uint8_t gameLevel = 1;
uint16_t voltageReadingTimer = 0;
uint16_t timer = 0;
...
void loop() {
  clearScreen();
  buttonDebounce();
  drawSprite (40, 1, letterL, 0);
  drawSprite (50, 1, letterE, 0);
  drawSprite (60, 1, letterV, 0);
  drawSprite (70, 1, letterE, 0);
  drawSprite (80, 1, letterL, 0);
  drawDecimal (54, 2, gameLevel);
...
void game (void) {
  ...
  while (!endGame) {
    ...
    if ((interruptTimer - enemiesTimer) > 80) { // enemies movement and kill logic
      ...
      if (enemiesPosX % 2 == 0) { // choose enemies animation based on if the pos is even or odd.
        enemiesPosXEven = true;
      } else {
        enemiesPosXEven = false;
      }

      for ( uint8_t x = 0; x < sizeof (enemiesPattern); x++) {
        enemyIsDead = false;

        for ( uint8_t y = 0; y < sizeof (enemiesKilled); y++) {
        ...
        if (!enemyIsDead) {
          ...
          if ( (enemiesPattern [x] + enemiesPosX) > 102 && enemiesMoveRight == true) {
          ...
          if (BUTTONLOW && enemiesFireAgain) { // choose wich enemy fires
          ...
          if (playerFirePosX > (enemiesPattern [x] + enemiesPosX) && playerFirePosX < (enemiesPattern [x] + enemiesPosX) + 8 && playerFirePosY == enemiesPosY) {
            ...
            drawDecimal (112, 2, enemiesKilledCounter);
          } else { 
            oled.drawLine((enemiesPattern [x] + lastEnemiesPosX), enemiesPosY, 8, 0x00); // clear last enemy position
            drawSprite(enemiesPattern [x] + enemiesPosX, enemiesPosY, enemiesAnimation [enemiesPosXEven], 0);
          }
        }
      }
    ...
    if ((interruptTimer - playerFireTimer) > 80) { // player fire logic
    ...
    if ((interruptTimer - playerTimer) > 15) { // player movement logic.
      ...
      drawSprite(playerPos, 3, ship, 0);
    }

    if (enemiesFirePosX > playerPos && enemiesFirePosX < (playerPos + 8) && enemiesFirePosY == 3 || enemiesKilledCounter > 5) { // checks if player is hit or not.
      endGame = true;
    }
  }
}
...
uint8_t voltageReading(void) {
  ...
  if ((interruptTimer - voltageReadingTimer) > 500) {
    actualVoltage = map(readSupplyVoltage(), 2600, 3800, 0, 99);

    if (actualVoltage < 0) actualVoltage = 0;
    voltageReadingTimer = interruptTimer;
  }

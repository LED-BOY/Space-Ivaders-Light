...
// draws "LEVEL" on screen.
  ...
  drawSprite (36, 0, tempArray, 0);
  drawSprite (78, 0, tempArray, 0);
  getSpriteFromEeprom(32, 40);// letters E
  drawSprite (46, 0, tempArray, 0);
  drawSprite (68, 0, tempArray, 0);
  getSpriteFromEeprom(40, 48);// letters v
  drawSprite (56, 0, tempArray, 0);
  drawDecimal (54, 2, gameLevel);

  if (gameLevel == 1) { // new HI score letters
    getSpriteFromEeprom(48, 56);// letters H
    drawSprite (112, 0, tempArray, 0);
    getSpriteFromEeprom(56, 64);// letters I
    drawSprite (120, 0, tempArray, 0);
...
void game (void) {
  ...
  while (true) {
    ...
    if ((interruptTimer - enemiesTimer) > (enemiesSpeed - gameLevel)) { // enemies movement and kill logic
...
void drawSprite (uint8_t column, uint8_t page, uint8_t sprite[], bool digit) {
  ...
  for (uint8_t x = 0; x < 8; x++) {
    ...
    if (digit && x > 5) {
      oled.ssd1306_send_data_byte(0x00); // some sprite are less than a Byte so 0 are sent to complete it and draw the screen properly
    } else {
